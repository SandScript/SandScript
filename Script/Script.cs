...
public class Script
{
	...
	public void AddClassMethods<T>() where T : class
	{
		...
		{
			if ( TypeProviders.GetByBackingType( method.ReturnType ) is null )
			...
			for ( var i = 1; i < methodParams.Length; i++ )
			{
				...
				if ( methodParam.ParameterType != typeof(ScriptValue) &&
				     TypeProviders.GetByBackingType( methodParam.ParameterType ) is null )
			...
			{
				var scriptMethod = new ScriptMethod( method, attribute );
